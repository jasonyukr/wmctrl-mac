...
`wmctrl-mac -m launch <app name>` opens a new window for a running app when supported, then falls back to launch-or-focus behavior. Running apps may be matched by exact or case-insensitive localized name or bundle id. For kitty, it tries remote-control OS-window creation using the resolved app bundle before falling back; for WezTerm, it tries `wezterm cli spawn --new-window` using the resolved app bundle, then activates the app; for other running apps with bundle ids, it tries AppleScript new-window commands before falling back. App names may be passed as one shell argument or multiple trailing arguments joined with spaces; paths are not supported.
